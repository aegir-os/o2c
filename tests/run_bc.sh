...
for name in sum ifelsif vmgreet proc local repeat case for set real arr rec ptr newt list recmix strch recreal outchar ext nested impderef fnexpr openarr typetest withguard typeguard dispatch newloop deepcall gcloop gcscalar ffi unops loopexit relops forstep fordown ptrfld ptrfield longarith arrparam boolops nobody filesintr lenopen nestedarr longfield strconst recarr; do
