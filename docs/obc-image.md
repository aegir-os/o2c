...
- **RECORD** — field list: repeated `{ name_ref, field_desc, offset }`;
  ...
  **The tag is also the size source**: a sweep walks the arena by taking each
  object's tag, following it to the descriptor and reading `size` there, so an
  object needs no length word of its own.  The tag word is part of whatever
  the object occupies, since the address handed out starts just past it.
...
| records | `LOAD_FLD_I`/`STORE_FLD_I` — a record variable is a run of scalar slots like an array, and a field is the word at the record's address plus its offset. The descriptor fixes the layout, so the offset is a compile-time constant and there is no runtime bound to check; what makes that safe is that the emitter can only name an offset the descriptor defined. CHAR, BOOLEAN, SET, REAL and LONGREAL fields share the same 8-byte slot, so only the opcode differs (`_I`, `_R`, `_P`) and only INTEGER and the pointer-sized ones are distinguished there. An extension's layout begins with its ancestors', so an inherited field keeps the offset it has in its parent and the variable's footprint is the whole chain's field count (`Total_Slots`), while a field's offset is composed by counting the fields of every record between the variable's type and the one that owns it (`Field_Offset`). A chain may also cross a user-typed field: a nested record's storage starts at that field's own offset, so the chain carries the offset it has walked and the leaf's offset and an indexed access's base are both relative to the value the chain reached, not to the variable. `Total_Slots` is recursive for the same reason - a nested record's field occupies its slots, not one - and both helpers bound their recursion so a record that nests without an intervening pointer is a refusal rather than a hang. Only scalar fields reached directly from a record variable for now — a chain of records would need the offsets composed, and an extension's layout is shared with its parent |
| pointers | `LOAD_CONST_P` (NIL), and a pointer variable is one scalar slot, so assignment is an ordinary store and `=`/`/=` compare two addresses with `EQ`/`NE` (only equality - an ordering gate refuses pointer relations). A *bare* pointer used as a value is pushed by the factor's user-typed-variable branch: every variable with a user type enters that branch before the scalar one, and with no selector following, no designator path pushes its base, so that branch pushes it. `p.f` is a load of the slot (which holds the record's address) plus a field offset, and `p^` adds `LOAD_IND_I`. `NEW` and the descriptor table it needs are implemented, together with `LOAD_FLD_P`/`STORE_FLD_P` for a field that holds a pointer. A field naming the record it belongs to is Oberon's implicit pointer: it types as one, defaults to null, and so a list can be built and walked - a chain may cross such a field, whose pointer is loaded as it goes. Oberon's implicit dereference is supported: `p.f` means `p^.f`, and costs nothing to emit because a pointer's value *is* the address of what it designates - only the static view moves. The one limit here is *spec-conformant* rather than ours: `=`/`#` between differently-named pointer types is refused, because the report's compatibility table requires the same type, or NIL, and defines no ordering on pointers at all. Extension-aware tests are the job of `IS`, not of `=` |
| allocation | `ALLOC_NEW` with a `TYPES` descriptor (section 3). Each descriptor is `kind`, `flags`, `size`, `name_ref` and a kind-specific tail; the operand names one by byte offset into that payload. RECORD descriptors are what `NEW` emits, sized `N_F * 8` because the layout is one scalar slot per field. **Sizes are slot counts, not byte counts**, so every allocation is a whole number of arena slots plus one more for the type tag: alignment is structural, a walk of the arena needs no padding rule, and an object's extent is `(size + 7) / 8 + 1` slots wherever it is computed. `TYPES` is optional and is only present when a program allocates, so an image that allocates nothing keeps the older layout. The VM takes the object zeroed from a bump-allocated arena and pushes its address - the same address the field and indexed accesses already consume - and has no collector and no reset (it is loaded and run once per process) |
...
| builtins | `CALL_NATIVE` (`Out.Int`, `Out.String`, `Out.Char`, `Out.Ln`, `Out.Real` - the VM formats a real exactly as `O2c_Put_Real` does, three digits after the point) |
| character literals | a literal of length one is a `CHAR`, not a string: this VM has always held a char as an integer in an 8-byte slot (`PUSH_CHAR` is `PUSH_INT`), and `LOAD_CONST` covers `INTEGER`/`CHAR`/`BOOLEAN`/`SET` alike. Only a longer literal is a string. `Out.String` accepts a char *literal*, folded to a one-character string at the call site |
